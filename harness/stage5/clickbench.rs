...
pub struct RunOpt {
    ...
    /// Experimental shared file-range read-ahead queue.
    #[arg(long, default_value_t = 0)]
    scan_read_ahead_jobs: usize,
    #[arg(long, default_value_t = 536870912)]
    scan_read_ahead_bytes: usize,
    #[arg(long, default_value_t = false)]
    scan_read_ahead_governed: bool,


    /// Synthetic latency per object-store read call (experiment only).
    #[arg(long, default_value_t = 0)]
    io_latency_ms: u64,
...
impl RunOpt {
    ...
    async fn benchmark_query(
        ...
        for i in 0..self.iterations() {
            let io_before = read_process_io();
            let cpu_before = read_process_cpu();
            ...
            if self.prefetch_bytes > 0 || self.io_latency_ms > 0 || self.scan_read_ahead_jobs > 0 {
                plan = plan
                    .transform_up(|plan| {
                                ...
                                exec.downcast_to_file_source::<ParquetSource>()
                        {
                            ...
                            if self.prefetch_bytes > 0 {
                                source = source.with_row_group_prefetch(
                                    self.prefetch_bytes,
                                    Arc::clone(&ctx.runtime_env().memory_pool),
                                );
                            }
                            if self.scan_read_ahead_jobs > 0 {
                                source = source.with_scan_read_ahead(self.scan_read_ahead_jobs,
                                    self.scan_read_ahead_bytes, self.scan_read_ahead_governed,
                                    Arc::clone(&ctx.runtime_env().memory_pool));
                            }
                            if self.io_latency_ms > 0 {
                                ...
                                let store = ThrottledStore::new(
                                    ...
                                    ThrottleConfig {
                                        wait_get_per_call: std::time::Duration::from_millis(self.io_latency_ms),
                                        ..Default::default()
                                    },
                                );
                                source = source.with_parquet_file_reader_factory(Arc::new(
                                    DefaultParquetFileReaderFactory::new(Arc::new(store)),
                                ));
                            }
            ...
            let io_after = read_process_io();
            let cpu_after = read_process_cpu();
            let io_delta = io_after.iter().map(|(key, value)| (key.clone(), value - io_before.get(key).copied().unwrap_or(0))).collect::<std::collections::BTreeMap<_, _>>();
            // Capture results and metrics after the timed query has finished.
            ...
            for name in [
                ...
                "prefetch_wait_time",
                "scan_read_ahead_jobs",
                "scan_read_ahead_initial_bytes",
                "scan_read_ahead_peak_bytes",
                "scan_read_ahead_peak_jobs",
                "scan_read_ahead_budget_denials",
                "scan_read_ahead_demand_fallbacks",
                "spill_count",
                "spilled_bytes",
            ] {
                let mut total = 0usize;
                plan.apply(|node| {
                    if let Some(metric) = node.metrics().and_then(|m| m.sum(|metric| metric.value().name() == name)) {
            ...
            println!(
                "PROFILE_QUERY {}",
                serde_json::json!({"pid":std::process::id(),"query_id":query_id,"iteration":i,"start_ns":start_ns,"elapsed_ms":elapsed.as_secs_f64()*1000.0,"rows":values.len(),"values":values,"types":results.first().map(|b| b.schema().fields().iter().map(|f|format!("{:?}",f.data_type())).collect::<Vec<_>>()),"counters":counters,"query_io":io_delta,"cpu_ticks":cpu_after-cpu_before})
            );
        ...
        if self.common.debug {
            ctx.sql(sql)
                .await?
                .explain_with_options(ExplainOption::default().with_format(ExplainFormat::Tree))?
                .show()
                .await?;
        }
...
fn read_process_io() -> std::collections::BTreeMap<String, u64> {
    fs::read_to_string("/proc/self/io").unwrap_or_default().lines()
        .filter_map(|line| { let (k,v) = line.split_once(':')?; Some((k.to_owned(),v.trim().parse().ok()?)) })
        .collect()
}

fn read_process_cpu() -> u64 {
    let stat = fs::read_to_string("/proc/self/stat").unwrap();
    let rest = stat.rsplit_once(") ").unwrap().1.split_whitespace().collect::<Vec<_>>();
